...
void drawColliderLayers(std::unique_ptr<shmup::SDLRenderer>& renderer,
  ...
  for (unsigned i = 0; i < enemyCount; ++i) {
    const shmup::Enemy& enemy = enemies[i];
    if (enemy.debugColliderPoints.empty() == false) {
      SDL_SetRenderDrawColor(renderer->native(), 255, 255, 255, 255);
      SDL_RenderDrawPointsF(renderer->native(),
                            enemy.debugColliderPoints.data(),
...
int main(int argc, char** argv) {
  ...
  // TODO: 플레이어 시작 지점 설정
  // const SDL_Point startPoint = { 10, 10 };
  const SDL_Point startPoint = {
      (int)(program->width() / 2 -
            (player->planeTexture()->header()->width / 2)),
      (int)(program->height() -
            player->planeTexture()->header()->height)};
  ...
  // program->updateTime(); // not necessary
  // Main loop
  while (program->neededQuit() == false) {
    ...
    drawBullets(renderer, player->bulletTexture(), player->bullets(), player->bulletCount());
